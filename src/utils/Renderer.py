...
class Renderer(object):
    ...
    def render_batch_ray(self, c, decoders, rays_d, rays_o, device, stage, gt_depth=None):
        ...
        if (gt_depth is not None) and (gt_depth.numel != 0):
        ...
        depth, uncertainty, color, weights, transmittance = raw2outputs_nerf_color(
        ...
        if N_importance > 0:
            ...
            depth, uncertainty, color, weights, transmittance = raw2outputs_nerf_color(
                raw, z_vals, rays_d, occupancy=self.occupancy, device=device)
            return depth, uncertainty, color, transmittance
