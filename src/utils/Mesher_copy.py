...
class Mesher(object):
    ...
    def __init__(self, cfg, args, slam, points_batch_size=500000, ray_batch_size=100000):
        ...
        self.marching_cubes_bound = torch.from_numpy(np.array(cfg['mapping']['marching_cubes_bound']))
    ...
    def get_scene_mesh(self, mesh_out_file, scene_model, device='cuda:0'):
        with torch.no_grad():

            bound = self.marching_cubes_bound


            x = torch.linspace(bound[0,0], bound[0,1], self.resolution) * self.scale
            y = torch.linspace(bound[1,0], bound[1,1], self.resolution) * self.scale
            z = torch.linspace(bound[2,0], bound[2,1], self.resolution) * self.scale

            query_pts = torch.stack(torch.meshgrid(x, y, z), -1)
            pts_flat = torch.reshape(query_pts, [-1, 3]).to(device)

            sample_points = self.batchify(scene_model, 1024*64)

            raw = sample_points(pts_flat)
            raw = torch.reshape(sample_points(pts_flat), list(query_pts.shape[:-1]) + [-1])
            density = F.relu(raw[..., 3])
            rgb = F.sigmoid(raw[..., :3])

            threshold = torch.std(density).cpu().numpy()

            vertices, faces, normals, _ = skimage.measure.marching_cubes(density.cpu().numpy(),
                                                                0.1 * threshold,
                                                                spacing=((bound[1,1]-bound[1,0]) / (self.resolution-1),
                                                                        (bound[0,1]-bound[0,0]) / (self.resolution-1),
                                                                        (bound[2,1]-bound[2,0]) / (self.resolution-1)),
                                                                allow_degenerate=False)
            vertices = np.array(vertices)[:,[1,0,2]] + np.array([[bound[0,0], bound[1,0], bound[2,0]]])
            normals = np.array(normals)[:,[1,0,2]]
            pts = torch.from_numpy(vertices).to(device)
            raw = sample_points(pts * self.scale)
            rgb_vertices = F.sigmoid(raw[..., :3]).cpu().numpy()

            mesh = trimesh.Trimesh(vertices, faces, vertex_normals=normals, vertex_colors=rgb_vertices)
            mesh.export(mesh_out_file)
