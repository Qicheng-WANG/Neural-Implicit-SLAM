...
def eval_mesh(file_pred, file_trgt, threshold=.05, down_sample=.02):
    ...
    metrics = {'dist1': np.mean(dist2),  # acc
               'dist2': np.mean(dist1),  # comp
               'prec': precision,
               'recal': recal,
               'fscore': fscore,
               }
    ...
    # scene = file_pred.split("/")[-3]
    # fieldnames = ["scene", "accuracy", "completion", "precision", "recal", "fscore"]
    # with open('output/scannet/scans/scannet_eval.csv', 'a+', newline='') as csvfile:
    #     writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    #     if scene == "scene0000_00":
    #         writer.writeheader()
    #     writer.writerow({"scene": scene, "accuracy": np.mean(dist2), "completion": np.mean(dist1), "precision": precision, "recal": recal, "fscore": fscore})
    # return metrics
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mesh',
                        action='store_true', help='enable mesh eval')
    parser.add_argument('--depth',
                        action='store_true', help='enable depth eval')
    args = parser.parse_args()
    if args.mesh:
        eval_mesh(args.rec, args.gt)

    if args.depth:
        eval_depth(args.rec, args.gt)
