...
def evaluate_ate(first_list, second_list, plot="", _args=""):
    ...
    parser.add_argument('--save_associations',
                        ...
                        default="output_imap/Replica/room0_BA/a.csv")
    parser.add_argument(
    ...
    if args.plot:
        ...
        ATE = numpy.sqrt(
            numpy.dot(trans_error, trans_error) / len(trans_error))
        ax.set_title(f'len:{len(trans_error)} ATE RMSE:{ATE:.6f} {args.plot[:-3].split("/")[2]}')
    ...
    # }
    scene = os.path.split(output)[-1]
    # scene = scene.split("_",2)[-1]
    return {
        "scene": scene,
        "compared_pose_pairs": (len(trans_error)),
        "ATE.rmse": numpy.sqrt(numpy.dot(trans_error, trans_error) / len(trans_error)),
        "ATE.mean": numpy.mean(trans_error),
        "ATE.median": numpy.median(trans_error),
        "ATE.std": numpy.std(trans_error),
        "ATE.min": numpy.min(trans_error),
        "ATE.max": numpy.max(trans_error),
    }
...
def evaluate(poses_gt, poses_est, plot):
    ...
    if args.save:
        scene = os.path.split(output)[-1]
        scene = scene.split("_",1)[0]
        # scene = scene.split("_",2)[-1]
        fieldnames = ["scene","compared_pose_pairs", "ATE.rmse", "ATE.mean", "ATE.median", "ATE.std", "ATE.min", "ATE.max"]
        # with open('output_imap/scannet_8layer/scans/scannet.csv', 'a+', newline='') as csvfile:
        # with open('output_imap/Replica/Replica_imap.csv', 'a+', newline='') as csvfile:
        with open(f'{os.path.split(output)[0]}/Replica_imap.csv', 'a+', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            # if scene == "freiburg1_desk":
            #     writer.writeheader()
            if output.split("/")[1].split("_")[0] == "scannet" and scene == "scene0000":
            # if scene == "scene0000":
                writer.writeheader()
            if output.split("/")[1].split("_")[0] == "Replica" and scene == "room0":
            # if scene == "room0":
                writer.writeheader()
