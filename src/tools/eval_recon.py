...
def completion_ratio(gt_points, rec_points, dist_th=0.05):
    ...
    '''DeprecationWarning: `np.float` is a deprecated alias for the builtin `float`. To silence this warning, use `float` by itself. 
...
def calc_3d_metric(rec_meshfile, gt_meshfile, align=True):
    ...
    if args.save:
        scene0 = rec_meshfile.split("/")[-3]
        scene = scene0.split("_",1)[0]
        fieldnames = ["scene", "accuracy", "completion", "completion ratio"]
        # with open('output/scannet/scans/scannet2.csv', 'a+', newline='') as csvfile:
        # with open('output_imap/Replica/Replica2_imap.csv', 'a+', newline='') as csvfile:
        with open(f'{os.path.abspath(os.path.join(rec_meshfile, "../../.."))}/Replica2_imap.csv', 'a+', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            if rec_meshfile.split("/")[1].split("_")[0] == "scannet" and scene == "scene0000":
                # if scene == "scene0000":
                writer.writeheader()
            elif rec_meshfile.split("/")[1].split("_")[0] == "Replica" and scene == "room0":
                # if scene == "room0":
                writer.writeheader()
            writer.writerow({"scene": scene0, "accuracy": accuracy_rec, "completion": completion_rec, "completion ratio": completion_ratio_rec})
...
def calc_2d_metric(rec_meshfile, gt_meshfile, align=True, n_imgs=1000):
    ...
    if rec_meshfile.split("/")[1] == "Replica":
        print("Replica")
    ...
    for i in range(n_imgs):
        while True:
            ...
            tmp[:3, :] = c2w
            c2w = tmp
            if rec_meshfile.split("/")[1].split("_")[0] == "Replica":
    ...
    if args.save:
        scene0 = rec_meshfile.split("/")[-3]
        scene = scene0.split("_",1)[0]
        fieldnames = ["scene", "Depth L1"]
        # with open('output/scannet/scans/scannet2.csv', 'a+', newline='') as csvfile:
        # with open('output_imap/Replica/Replica2_imap.csv', 'a+', newline='') as csvfile:
        with open(f'{os.path.abspath(os.path.join(rec_meshfile, "../../.."))}/Replica1_imap.csv', 'a+', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            if rec_meshfile.split("/")[1].split("_")[0] == "scannet" and scene == "scene0000":
                # if scene == "scene0000":
                writer.writeheader()
                ...
                # if scene == "room0":
                writer.writeheader()
            writer.writerow({"scene": scene, "Depth L1": errors.mean()*100})
