...
def get_model(cfg,  nice=True):
    ...
    if nice:
        ...
        decoder = models.decoder_dict['imap'](
            ...
            hidden_size=256, skips=[1], n_blocks=4, pos_embedding_method=pos_embedding_method
        )
        """I think the skips should be [1], since the embedding is concatenating to the second avtivation layer 👆👆👆"""
        # decoder = models.decoder_dict['imap'](
        #     dim=dim, c_dim=0, color=True,
        #     hidden_size=256, skips=[4], n_blocks=8, pos_embedding_method=pos_embedding_method
        # )
    return decoder
