...
def sample_pixels_from_regions(H0, H1, W0, W1, regions, n_samples, device):
    region_h = (H1 - H0) // 8
    region_w = (W1 - W0) // 8
    i = (regions // 8) * region_h + torch.randint(region_h, (n_samples,), device=device)
    j = (regions % 8) * region_w + torch.randint(region_w, (n_samples,), device=device)
    return i, j


def get_samples_uniform(H0, H1, W0, W1, n_samples, H, W, fx, fy, cx, cy, c2w, depth, color, device):
    depth = depth[H0:H1, W0:W1]
    color = color[H0:H1, W0:W1]
    numbers = torch.arange(n_samples).to(device)
    regions = numbers % 64
    i, j = sample_pixels_from_regions(H0, H1, W0, W1, regions, n_samples, device)
    sample_depth = depth[i,j]
    sample_color = color[i,j]
    rays_o, rays_d = get_rays_from_uv(j, i, c2w, H, W, fx, fy, cx, cy, device)
    return rays_o, rays_d, sample_depth, sample_color


def get_samples_guided(H0, H1, W0, W1, loss_dist, n_samples, H, W, fx, fy, cx, cy, c2w, depth, color, device):
    depth = depth[H0:H1, W0:W1]
    color = color[H0:H1, W0:W1]
    samples = torch.distributions.Categorical(
                        logits = torch.log(loss_dist)).sample([n_samples])
    regions = samples % 64
    i, j = sample_pixels_from_regions(H0, H1, W0, W1, regions, n_samples, device)
    sample_depth = depth[i,j]
    sample_color = color[i,j]
...
def raw2outputs_nerf_color(raw, z_vals, rays_d, occupancy=False, device='cuda:0', raw_noise_std=1e0):
    """
    ...
    if occupancy:
        ...
        alpha = raw2alpha(raw[..., -1] + noise.to(device), dists)  # (N_rays, N_samples)

    transmittance = (1.-alpha + 1e-10).float()
    transmittance_acc = torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)).to(
        device).float(), transmittance], -1).float(), -1)[:, :-1]
    weights = alpha.float() * transmittance_acc
    ...
    return depth_map, depth_var, rgb_map, weights, ray_transmittance
