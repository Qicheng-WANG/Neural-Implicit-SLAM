...
from src.common import (get_camera_from_tensor, get_samples,
                        get_samples_guided, get_samples_uniform,
...
class Mapper(object):
    ...
    def __init__(self, cfg, args, slam, coarse_mapper=False
        ...
        self.prev_scene_model = copy.deepcopy(self.decoders)
    ...
    def optimize_map(self, num_joint_iters, lr_factor, idx, cur_gt_color, cur_gt_depth, gt_cur_c2w, keyframe_dict, keyframe_list, cur_c2w, init, outer_joint_iter):
        """
        ...
        if len(keyframe_dict) == 0:
            ...
            if self.keyframe_selection_method == 'global':
                ...
                optimize_frame = torch.distributions.Categorical(
                    logits = torch.log(torch.tensor(self.keyframes_loss_distribution))).sample([num]).tolist()
        ...
        optimize_frame += [-1]
        # print("optimize_frame:{}".format(optimize_frame))

        # oldest_frame = None
        # if len(keyframe_list) > 0:
        #     optimize_frame = torch.concat(
        #         [optimize_frame, torch.tensor(len(keyframe_list)-1).unsqueeze(0)], 0)
        #     oldest_frame = min(optimize_frame)
        # optimize_frame = torch.concat(
        #         [optimize_frame, torch.tensor(-1).unsqueeze(0)], 0)
        ...
        # imap*, single MLP
        if self.BA:
            optimizer1 = torch.optim.Adam([{'params': decoders_para_list, 'lr': 0},
        ...
        scheduler = StepLR(optimizer1, step_size=200, gamma=0.8)

        optimizer1.param_groups[0]['lr'] = cfg['mapping']['imap_decoders_lr']
        # optimizer2.param_groups[0]['lr'] = cfg['mapping']['imap_decoders_lr']
        if self.BA:
            optimizer1.param_groups[1]['lr'] = self.BA_cam_lr
        ...
        for joint_iter in range(num_joint_iters):
            ...
            # if (not (idx == 0 and self.no_vis_on_first_frame)) and ('Demo' not in self.output):
            #     self.visualizer.vis(
            #         idx, outer_joint_iter, cur_gt_depth, cur_gt_color, cur_c2w, self.c, self.decoders)
            ...
            for frame in optimize_frame:
                ...
                batch_rays_o, batch_rays_d, batch_gt_depth, batch_gt_color = get_samples_uniform(
            ...
            # print("depth:{}".format(gt_depth.size()))
            # print("color:{}".format(gt_color.size()))
            ...
            depth, uncertainty, color, transmittance = ret

            # mask for those out of bounding box depth value
            with torch.no_grad():
                det_rays_o = batch_rays_o.clone().detach().unsqueeze(-1)  # (N, 3, 1)
                det_rays_d = batch_rays_d.clone().detach().unsqueeze(-1)  # (N, 3, 1)
                t = (self.bound.unsqueeze(0).to(device)-det_rays_o)/det_rays_d
                t, _ = torch.min(torch.max(t, dim=2)[0], dim=1)
                outside_mask = t < batch_gt_depth
            ...
            # free space loss
            losses_freespace = torch.where(outside_mask, (1. - transmittance).double(), 0.).sum()
            loss += 0.5 * losses_freespace


            '''新加'''
            # losses_dist = torch.where(depth_mask, torch.abs(batch_gt_depth-depth), 0.) +\
            #             torch.abs(batch_gt_color - color).sum(dim=1)
            losses_dist = torch.where(depth_mask, torch.abs(batch_gt_depth-depth), 0.) +\
                        torch.where(depth_mask, torch.abs(batch_gt_color-color).sum(dim=1).double(), 0.)
            loss_dist = torch.mean(torch.reshape(losses_dist, (len(optimize_frame), 64, -1)), axis=-1)
        ...
        # # losses_dist = torch.where(depth_mask, torch.abs(batch_gt_depth-depth), 0.) + \
        # #     torch.abs(batch_gt_color-color).sum(dim=1)
        # loss_dist = torch.abs(batch_gt_depth[depth_mask]-depth[depth_mask]) +\
        #                 torch.abs(batch_gt_color - color).sum(dim=1)
        # img_loss_dist = torch.mean(losses_dist.reshape(len(optimize_frame), -1), dim=-1)

        # for id, frame in enumerate(optimize_frame[:-1]):
        #     self.keyframes_loss_distribution[frame] = img_loss_dist[id].detach()


            
            if not init:
            # if self.BA:
                '''根据region loss来sample不同region点的数量'''
                ...
                for id, frame in enumerate(optimize_frame):
                    ...
                    batch_rays_o, batch_rays_d, batch_gt_depth, batch_gt_color = get_samples_guided(
                        0, H, 0, W, loss_dist[id], pixs_per_image, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, self.device)
                    batch_rays_o_list.append(batch_rays_o.float())
                    batch_rays_d_list.append(batch_rays_d.float())
                    batch_gt_depth_list.append(batch_gt_depth.float())
                    batch_gt_color_list.append(batch_gt_color.float())
        ...
        if self.BA:
            return cur_c2w, img_loss_dist
        else:
            return img_loss_dist
    ...
    0, H, 0, W, n_rays, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, self.device)
        ...
        depth, uncertainty, color, transmittance = self.renderer.render_batch_ray(self.c, scene_model, batch_rays_d,
                                        batch_rays_o, self.device, self.stage,
                                        batch_gt_depth)
        depth_mask = (batch_gt_depth > 0)
        losses_depth = torch.abs(batch_gt_depth[depth_mask]-depth[depth_mask])
        losses_depth_normalized = losses_depth / batch_gt_depth[depth_mask]
    ...
    def should_add_new_keyframe(self, idx, gt_color, gt_depth, cur_c2w):
        information_gain = self.check_information_gain(self.prev_scene_model,
                                                    gt_color,
                                                    gt_depth,
                                                    cur_c2w,
                                                    400)
        print("information_gain:{}".format(information_gain))
        frames_elapsed = idx - self.keyframe_list[-1]

        # return information_gain < 0.65 and frames_elapsed > self.keyframe_every
        # return (information_gain < 0.75 and frames_elapsed >= 15)
        return (information_gain < 0.75 and (frames_elapsed >= 15 or idx <= 15))
        ...
        while (1):
            while True:
                ...
                if self.sync_method == 'strict':
                    ...
                    if (idx % self.every_frame == 0 or idx < 13) and idx != prev_idx:
                        break
            ...
            if not init:
                ...
                if idx == self.n_img-1 and self.color_refine and not self.coarse_mapper:
                    ...
                    if self.nice:
                        ...
                        outer_joint_iters = 3  ### By Default
                        # outer_joint_iters = 1
            ...
            # for joint_iter in range(num_joint_iters):
                ...
                self.BA = (len(self.keyframe_list) > 0) and cfg['mapping']['BA'] and (
                    not self.coarse_mapper)
                # self.BA = cfg['mapping']['BA'] and (not self.coarse_mapper)

                _ = self.optimize_map(num_joint_iters, lr_factor, idx, gt_color, gt_depth,gt_c2w, 
                                      self.keyframe_dict, self.keyframe_list, cur_c2w, init, outer_joint_iter)
                if self.BA:
                    cur_c2w, img_loss_dist = _
            ...
            '''改改改
                改改
                改改'''
            if init or self.should_add_new_keyframe(idx, gt_color, gt_depth, cur_c2w):
                # self.add_keyframe(idx, gt_c2w, gt_color, gt_depth, cur_c2w)
                print("adding keyframe:{}".format(idx))
                self.keyframe_list.append(idx)
                print("keyframe list:{}".format(self.keyframe_list))
                self.keyframe_dict.append({'gt_c2w': gt_c2w.cpu(), 'idx': idx, 'color': gt_color.cpu(
                ), 'depth': gt_depth.cpu(), 'est_c2w': cur_c2w.clone()})
                self.keyframes_loss_distribution.append(img_loss_dist[-1].detach())
                self.prev_scene_model = copy.deepcopy(self.decoders)
            ...
            if not self.coarse_mapper:
                ...
                # if (idx % self.mesh_freq == 0) and (not (idx == 0 and self.no_mesh_on_first_frame)):
                #     mesh_out_file = f'{self.output}/mesh/{idx:05d}_mesh.ply'
                #     self.mesher.get_scene_mesh(mesh_out_file, self.decoders, self.device)
